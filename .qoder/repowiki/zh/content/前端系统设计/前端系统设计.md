...
## 更新摘要
**所做更改**
- 更新了项目结构描述，反映完整的多角色界面实现
- 补充了教师端和管理员端页面的详细分析
- 完善了状态管理、API集成和组件化开发的说明
- 增强了页面适配和兼容性的最佳实践指导
- 更新了性能优化和故障排查指南
...
## 简介
本设计文档面向 HelenEdu 前端系统，基于 Vue 3 + UniApp 的多端开发架构，统一支持微信小程序与 H5 网页端。系统已完成完整的多角色界面实现，包括学生端、教师端和管理员端的功能模块。文档从系统架构、目录结构、页面路由、状态管理、API 集成、组件化开发、样式与主题、页面适配与兼容、开发调试与性能优化等方面进行系统化说明，帮助开发者快速理解与扩展系统。

## 项目结构
前端工程位于 helenedu-frontend 目录，采用典型的 UniApp 工程组织方式，现已完整实现多角色界面：
- 应用入口与全局配置：main.js、App.vue、pages.json、vite.config.js
- 页面与功能模块：按角色划分的 pages 子目录（admin、teacher、student），以及登录页
- 状态管理：store/user.js 使用 Pinia 管理用户状态
...
H["请求封装<br/>utils/request.js"] --> I["业务接口聚合<br/>api/index.js / api/auth.js"]
E --> H
C --> H
```
...
- [pages.json:1-120](file://helenedu-frontend/src/pages.json#L1-L120)
...
TL["教师作业列表<br/>pages/teacher/homework-list.vue"]
TC["教师发布作业<br/>pages/teacher/homework-create.vue"]
AD["管理员看板<br/>pages/admin/dashboard.vue"]
AC["管理员班级管理<br/>pages/admin/class-manage.vue"]
end
subgraph "业务层"
...
**图表来源**
- [login/index.vue:1-192](file://helenedu-frontend/src/pages/login/index.vue#L1-L192)
- [student/homework-list.vue:1-197](file://helenedu-frontend/src/pages/student/homework-list.vue#L1-L197)
- [student/homework-submit.vue:1-218](file://helenedu-frontend/src/pages/student/homework-submit.vue#L1-L218)
- [teacher/homework-list.vue:1-165](file://helenedu-frontend/src/pages/teacher/homework-list.vue#L1-L165)
- [teacher/homework-create.vue:1-351](file://helenedu-frontend/src/pages/teacher/homework-create.vue#L1-L351)
- [admin/dashboard.vue:1-138](file://helenedu-frontend/src/pages/admin/dashboard.vue#L1-L138)
- [admin/class-manage.vue:1-66](file://helenedu-frontend/src/pages/admin/class-manage.vue#L1-L66)
- [index.js:1-50](file://helenedu-frontend/src/api/index.js#L1-L50)
- [auth.js:1-8](file://helenedu-frontend/src/api/auth.js#L1-L8)
- [request.js:1-134](file://helenedu-frontend/src/utils/request.js#L1-L134)
- [user.js:1-63](file://helenedu-frontend/src/store/user.js#L1-L63)
- [pages.json:1-120](file://helenedu-frontend/src/pages.json#L1-L120)
...
- TabBar：定义学生端的作业、预习、我的三个 Tab，含图标与选中态图标路径
...
### 用户状态管理（Pinia Store）
- 状态字段：token、userInfo（含 id、name、role、avatarUrl、subject）
...
- 作业相关：列表、详情、创建、更新、删除、提交、查看提交记录、批改、提交详情
- 预习资料：列表、详情、创建、更新、删除
...
#### 教师作业管理页面
- 功能点：作业列表展示、状态统计、详情查看、批改入口、学生管理
- 逻辑：展示作业统计信息，支持查看详情和批改操作

**章节来源**
- [teacher/homework-list.vue:1-165](file://helenedu-frontend/src/pages/teacher/homework-list.vue#L1-L165)

#### 教师发布作业页面
- 功能点：表单填写、日期选择、图片/文件附件上传、指定学生范围
- 逻辑：组装参数并调用创建接口，支持编辑模式
...
#### 管理员班级管理页
- 功能点：班级列表、新建/编辑、删除操作
- 逻辑：展示班级信息，支持 CRUD 操作

**章节来源**
- [admin/class-manage.vue:1-66](file://helenedu-frontend/src/pages/admin/class-manage.vue#L1-L66)

## 依赖关系分析
- 构建与多端：package.json 中定义 dev/build 脚本，集成 @dcloudio/vite-plugin-uni 插件
...
- 条件编译：针对不同平台优化用户体验，如 H5 端使用原生文件选择器
...
## 结论
本系统以 Vue 3 + UniApp 为基础，结合 Pinia 状态管理与统一请求封装，实现了跨小程序与 H5 的一致体验。通过 pages.json 统一路由与 TabBar，配合条件编译与本地存储策略，满足多端开发需求。系统已完成完整的多角色界面实现，包括学生端的作业管理、教师端的班级管理和作业发布、管理员端的数据看板等功能模块。后续可在接口幂等、错误重试、缓存策略与主题变量化方面进一步优化。
...
**章节来源**
- [pages.json:2-85](file://helenedu-frontend/src/pages.json#L2-L85)