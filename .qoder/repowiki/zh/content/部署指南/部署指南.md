...
## 更新摘要
**所做更改**
- 新增完整的阿里云部署指南文档，包含服务器环境搭建、数据库配置、Nginx部署等详细步骤
- 添加跨平台一键启动脚本（Windows和Linux），简化开发环境部署
- 更新部署流程以反映实际可用的部署文档和脚本
- 完善生产环境配置示例和运维命令
...
**更新** 本指南现已整合完整的阿里云部署文档和跨平台启动脚本，提供了从开发环境到生产环境的全流程部署方案。
...
- [vite.config.js:1-12](file://helenedu-frontend/vite.config.js#L1-L12)
- [package.json:1-28](file://helenedu-frontend/package.json#L1-L28)

**章节来源**
- [DEPLOY.md:1-322](file://DEPLOY.md#L1-L322)
- [start.sh:1-84](file://start.sh#L1-L84)
- [stop.sh:1-35](file://stop.sh#L1-L35)
- [start.bat:1-87](file://start.bat#L1-L87)
- [stop.bat:1-23](file://stop.bat#L1-L23)
...
- 一键启动脚本：提供Windows和Linux平台的跨平台部署脚本，支持自动停止现有服务、启动后端和前端服务。

**更新** 新增完整的部署文档和跨平台启动脚本，提供了从开发到生产的完整部署方案。
...
### 一键启动脚本系统
- Linux启动脚本(start.sh)：自动停止现有服务、启动后端(8888端口)和前端(5173端口)，并提供日志查看功能
- Windows启动脚本(start.bat)：支持MySQL服务检查、自动启动后端和前端，集成浏览器自动打开
- 停止脚本：提供对应的停止功能，清理PID文件
- 跨平台支持：统一的部署流程，简化开发环境配置

**更新** 新增完整的跨平台启动脚本系统，提供一键部署能力。
...
- 一键启动脚本问题
  - 权限问题：确保脚本具有执行权限
  - 端口冲突：检查8888和5173端口是否被占用
  - 依赖缺失：确认Java、Node.js、MySQL服务已正确安装

**更新** 新增一键启动脚本相关的故障排查指南。
...
## 结论
本部署指南基于仓库现有配置与代码，给出了从环境准备、编译打包、数据库初始化、配置设置、服务启动验证到容器化、反向代理与SSL、监控与日志、备份恢复、问题排查与自动化集成的完整路径。**更新** 通过新增的阿里云部署文档和跨平台启动脚本，提供了更加完善的部署体验。建议在生产环境中补充安全加固、缓存与CDN、APM监控与告警体系，并制定严格的变更与回滚流程。
...
- 一键启动（开发环境）
  - Linux：./start.sh（自动停止现有服务并启动新服务）
  - Windows：start.bat（自动检查MySQL并启动服务）
...
### 八、阿里云部署完整指南
- 服务器规格：通用算力型u1，2核CPU，4GB内存，80GB系统盘
- 系统要求：Ubuntu 22.04 LTS
- 端口配置：22(SSH)、80(Nginx)、443(HTTPS)、8080(后端API)、3306(MySQL)
- 部署步骤：
  - 系统更新与JDK安装
  - MySQL 8.0安装与安全配置
  - Nginx安装与H5部署
...
**更新** 新增完整的阿里云部署指南，包含详细的服务器配置和部署步骤。
...
- Linux系统
  ...
  - 启动：./start.sh（自动停止现有服务并启动新服务）
  - 停止：./stop.sh
  - 日志：查看/tmp/helenedu-backend.log和/tmp/helenedu-frontend.log
- Windows系统
  - 双击start.bat启动（需要管理员权限）
  - 自动检查MySQL服务状态
  - 支持自动打开浏览器访问前端
  - 停止：双击stop.bat
